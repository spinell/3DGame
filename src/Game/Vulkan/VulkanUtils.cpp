...
auto std::formatter<VkResult>::format(VkResult result, format_context& ctx) const
    -> decltype(ctx.out()) {
    return std::format_to(ctx.out(), "{}", string_VkResult(result));
}

auto std::formatter<uint8_t[VK_UUID_SIZE]>::format(uint8_t         uuid[VK_UUID_SIZE],
...
auto std::formatter<VkLayerProperties>::format(VkLayerProperties layer, format_context& ctx) const
    -> decltype(ctx.out()) {
    return std::format_to(
...
auto std::formatter<VkExtensionProperties>::format(VkExtensionProperties extension,
                                                   format_context&       ctx) const
    -> decltype(ctx.out()) {
...
auto std::formatter<VkConformanceVersion>::format(VkConformanceVersion version,
                                                  format_context&      ctx) const
    -> decltype(ctx.out()) {
...
auto std::formatter<VkDriverId>::format(VkDriverId id, format_context& ctx) const
    -> decltype(ctx.out()) {
    return std::format_to(ctx.out(), "{}", string_VkDriverId(id));
}

auto std::formatter<VkExtent2D>::format(VkExtent2D extent, format_context& ctx) const
    -> decltype(ctx.out()) {
    return std::format_to(ctx.out(), "{}x{}", extent.width, extent.height);
}
auto std::formatter<VkExtent3D>::format(VkExtent3D extent, format_context& ctx) const
    -> decltype(ctx.out()) {
